"""Fetch OHLCV + fundamentals for GateForum.s research pairs and cache them for the tick.
...
class Config(BaseModel):
    ...
    market_source: str = Field(
        default="gate_io",
        description="Public market-data source for candles: gate_io or bitget",
    )
...
async def _fetch_candles(
    ...
    try:
        ...
        logger.warning("gateforum_data: candle fetch failed for %s: %s", pair, exc)
        return []
...
async def _fetch_fundamentals(
    ...
    try:
        ...
        logger.warning("gateforum_data: coingecko failed for %s: %s", base, exc)
...
async def run(config: Config, context: ContextTypes.DEFAULT_TYPE):
    """Gather data for every research pair, persist it, and render the report."""
    pairs = _parse_pairs(config.pairs)
    if not pairs:
        return "gateforum_data: no pairs configured."
    ...
    # Persist for gateforum_research + the tick. manage_memory is the current store;
    ...
    for item in results:
        ...
        try:
            await memory.manage_memory(
                action="write",
                name=f"gateforum_market_{item['pair'].replace('-', '_')}",
                content=json.dumps(item),
                description=f"GateForum market snapshot for {item['pair']}",
                type="reference",
            )
        except Exception as exc:  # noqa: BLE001
            logger.warning("gateforum_data: memory write failed for %s: %s", item["pair"], exc)
    ...
    try:
        ...
        builder = ReportBuilder("GateForum — Market Intelligence")
        builder.source("routine", "gateforum_data")
        builder.tags(["gateforum", "market-data", config.market_source])
        ...
        builder.section(
            "01 / PAIR SNAPSHOT",
            "Price, momentum and trend for each asset entering the research.",
        )
        builder.table(rows, columns)
        builder.section(
            "02 / BRIEFING PACKETS",
            "Exact text handed to the analyst agents on the research floor.",
        )
        for item in results:
        ...
        logger.warning("gateforum_data: report generation failed: %s", exc)

    from routines.base import RoutineResult

    summary = (
        f"GateForum data refreshed from {config.market_source}: {ok}/{len(pairs)} pairs "
