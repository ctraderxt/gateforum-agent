"""Live smoke test of the GateForum research engine against opencode-go / deepseek-v4-flash.
...
Run with the condor venv so CUSTOM_LLM_BASE_URL / OPENCODE_GO_API_KEY are present:
  uv run python agents/gateforum/tests/live_research.py
"""
...
async def main() -> int:
    ...
    for pair in PAIRS:
        print(f"\n=== LIVE DEBATE: {pair} ===")
        try:
            res = await run_research(pair, PACKET, cfg)
        ...
        if res["decision"] in ("BUY", "SELL", "HOLD") and res["rationale"]:
            ok += 1
    print(f"\n=== RESULT: {ok}/{len(PAIRS)} live research returned a real verdict ===")
