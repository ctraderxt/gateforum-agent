"""Validate the gateforum agent against Condor's real loaders (no network, no trading)."""
import sys
...
rdir = REPO / "agents/gateforum/routines"
found = discover_routines_from_path(rdir, agent_slug="gateforum")
...
for expected in ("gateforum_init", "gateforum_data", "gateforum_research"):
...
agent = AgentStore().get("gateforum")
if not agent:
    print("  [FAIL] agent 'gateforum' not loaded"); ok = False
else:
...
strats = [s for s in StrategyStore().list_all() if s.agent_slug == "gateforum"]
if not strats:
    print("  [FAIL] no strategy loaded for gateforum"); ok = False
for s in strats:
