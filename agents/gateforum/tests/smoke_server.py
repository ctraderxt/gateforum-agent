"""Boot the GateForum research server in mock mode and exercise the full HTTP contract.

Verifies /health, a real /research (full transcript), and /history — proving the
server, engine and Condor routine contract line up end to end, with no LLM key.
Run: uv run python agents/gateforum/tests/smoke_server.py
"""
...
import uvicorn
from gateforum_server import app  # import after sys.path is set
...
async def _main() -> int:
    ...
    async with httpx.AsyncClient() as client:
        ...
        # payload mirrors what gateforum_research.py sends
        payload = {
        ...
        d = await client.post(f"{BASE}/research", json=payload, timeout=30)
        if d.status_code != 200:
            print(f"[FAIL] /research -> {d.status_code} {d.text[:200]}"); return 1
        body = d.json()
        print(f"[OK] /research -> {body['decision']} {body['direction']} conf={body['confidence']}%")
        ...
        if missing:
            print(f"[FAIL] /research missing fields: {missing}"); return 1
